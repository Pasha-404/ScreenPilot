...
| Переход из clone или неактивного target к Extend | NOT TESTED | Реальный экран во время Stage 5 уже работал в Extend. Нужен отдельный ручной тест с clone/неактивным HDMI target и наблюдением того, что Windows временно переходит к раздельным desktop. |
