...
| Переход из clone или неактивного target к Extend | NOT TESTED | Реализация сохраняет исходный snapshot и допускает target без отдельного GDI-имени/режима; после `SetDisplayConfig(SDC_TOPOLOGY_EXTEND)` она повторно находит именно тот же `adapter LUID + target ID`, иначе делает rollback. Unit-тесты покрывают distinction Extend/clone/inactive. Нужен отдельный ручной тест с clone или неактивным HDMI target. |
| Отключение HDMI во время polling | NOT TESTED | Реализована read-only команда `display-hot-unplug-watch --hold-ms=30000`; smoke без отключения выполнился. Кабель во время проверки не отсоединялся. Нужна отдельная ручная проверка обнаружения не позднее 2 секунд. |
