...
| UI этапа 6: повторный запуск после «Стоп» и смена выбранного режима | NOT TESTED | Исправления подготовлены после отчёта 07.08.2026: `Stop` оставляет mpv в `OUTPUT_IDLE`, а UI сохраняет подтверждённый ручной режим. Нужна повторная проверка с HDMI. |
| UI этапа 7: плейлист, auto-next и resume | NOT TESTED | Логика, persistence и FXML проверены автоматическими тестами. Реальный переход между файлами и диалог продолжения на external output пока не проверялись по запросу владельца проекта. |
| UI этапа 7: внешние субтитры и drag-and-drop | NOT TESTED | JavaFX controls и команда mpv `sub-add` реализованы; требуется ручная проверка с локальным видео и HDMI-output. |
