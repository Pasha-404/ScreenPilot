...
- Runtime намеренно исключён из Git и перед сборкой восстанавливается только из закреплённого, hash-checked архива. Неполный статус notices зафиксирован в `vendor/mpv/THIRD_PARTY_NOTICES.md`: перед первым публичным тегом нужен полный комплект для точной сборки. Это не препятствует локальной сборке и тестированию установщика.
