...
| GitHub Actions: сборка и GitHub Release | NOT TESTED | Тег `v0.1.1` подтвердил успешную cloud-сборку, но выявил ошибку workflow: в задаче публикации отсутствовал checkout скрипта проверки. Теги `v0.1.2` и `v0.1.3` показали кратковременное закрытие mpv IPC в integration-тесте Windows runner. Проверка повторяется на новом неизменяемом теге `v0.1.4`; hardware-тесты запускаются только с `-PincludeHardwareTests=true`. |
