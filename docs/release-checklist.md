...
| Per-user EXE installer | PASS | 17.09.2026 Inno Setup 6.7.1 собрал `ScreenPilot-Setup-0.1.3-x64.exe` из self-contained app-image. Это подтверждает упаковку, но не заменяет проверку установки. |
...
| GitHub Actions: сборка и GitHub Release | NOT TESTED | Тег `v0.1.1` подтвердил успешную cloud-сборку, но выявил ошибку workflow: в задаче публикации отсутствовал checkout скрипта проверки. Тег `v0.1.2` показал кратковременное закрытие mpv IPC в integration-тесте Windows runner. Проверка повторяется на новом неизменяемом теге `v0.1.3`; hardware-тесты запускаются только с `-PincludeHardwareTests=true`. |
