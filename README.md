...
Версия: **0.1.2**. Целевая система — Windows 10/11 x64. Приложение не меняет системное устройство звука по умолчанию и не включает HDR Windows.
...
```powershell
.\gradlew.bat clean buildWindowsInstaller "-Pversion=0.1.2"
```

В `dist\release\0.1.2\` появятся ровно три обязательных файла:

- `ScreenPilot-Setup-0.1.2-x64.exe`;
- `ScreenPilot-Setup-0.1.2-x64.exe.sha256`;
...
.\gradlew.bat --no-daemon clean buildWindowsInstaller "-Pversion=0.1.2"
```
...
Проверки с тегом `hardware` по умолчанию исключены: GitHub Actions не имеет подключённого физического экрана. Их запускают только на подходящем ноутбуке с явным флагом `-PincludeHardwareTests=true`.
